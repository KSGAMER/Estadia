...
public class ModeloIncidencias extends BD {
    ...
    protected DefaultTableModel cargarTabla(String filtro) {
        ...
        try {
            this.st = conectar().prepareStatement("SELECT i.Nombre, i.Observaciones FROM Incidencias i INNER JOIN Habitacion h on h.IdHabitacion = i.IdHabitacion WHERE h.Nombre like CONCAT('%',?,'%') and i.IdEstadoIncidencia = 2");
