...
public class ControladorPrivilegiosHabitaciones {
                             ...
                             if (modulo.getNombre().equals(NombreModulo)) {
                                ...
                                if (permiso.getConsultar() == estado.getIdEstatusPermiso()) {
                                    if (estado.getNombre().equals("Permitido")) {
                                        if (Pn_NuevaHabitacion.jt_Buscar.isEnabled() && Pn_NuevaHabitacion.jt_Buscar.isEditable()) {
                                            //si esta activado no hara nada
                                        } else {
                                            //si el boton estaba desactivado no activará
                                            Pn_NuevaHabitacion.jt_Buscar.setEnabled(true);
                                            Pn_NuevaHabitacion.jt_Buscar.setEditable(true);
                                        }
                                    } else if (estado.getNombre().equals("Denegado")) {
                                        //no importa su estado , desactivará el textfield
                                        Pn_NuevaHabitacion.jt_Buscar.setEnabled(false);
                                        //no importa su estado , no se podrá escribir
                                        Pn_NuevaHabitacion.jt_Buscar.setEditable(false);
                                        Pn_NuevaHabitacion.jt_Buscar.addMouseListener(new MouseListener() {
                                            @Override
                                ...
                                if (permiso.getInsertar() == estado.getIdEstatusPermiso()) {
                                    if (estado.getNombre().equals("Permitido")) {
                                        if (Pn_NuevaHabitacion.btn_Ingresar.isEnabled()) {
                                            //si esta activado no hara nada
                                        } else {
                                            //si el boton estaba desactivado no activará
                                            Pn_NuevaHabitacion.btn_Ingresar.setEnabled(true);
                                        }
                                    } else if (estado.getNombre().equals("Denegado")) {
                                        
                                        //no importa su estado , desactivará al boton
                                        Pn_NuevaHabitacion.btn_Ingresar.setEnabled(false);
                                    }
                                }
                                //VALIDAR PERMISO DE ACTUALIZACION
                                if (permiso.getActualizar() == estado.getIdEstatusPermiso()) {
                                    if (estado.getNombre().equals("Permitido")) {
                                        if (Pn_NuevaHabitacion.btn_Modificar.isEnabled()) {
                                            //si esta activado no hara nada
                                        } else {
                                            //si el boton estaba desactivado no activará
                                            Pn_NuevaHabitacion.btn_Modificar.setEnabled(true);
                                        }
                                    } else if (estado.getNombre().equals("Denegado")) {
                                        //no importa su estado , desactivará al boton
                                        Pn_NuevaHabitacion.btn_Modificar.setEnabled(false);
                                    }
                                }
                                //VALIDAR PERMISO DE ELIMINACION
                                if (permiso.getEliminar()== estado.getIdEstatusPermiso()) {
                                    if (estado.getNombre().equals("Permitido")) {
                                        if (Pn_NuevaHabitacion.btn_Eliminar.isEnabled()) {
                                            //si esta activado no hara nada
                                        } else {
                                            //si el boton estaba desactivado no activará
                                            Pn_NuevaHabitacion.btn_Eliminar.setEnabled(true);
                                        }
                                    } else if (estado.getNombre().equals("Denegado")) {
                                        //no importa su estado , desactivará al boton
                                        Pn_NuevaHabitacion.btn_Eliminar.setEnabled(false);
                                    }
                                }
                            }
